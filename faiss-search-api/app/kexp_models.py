...
class PaginatedResponse[T](BaseModel):
    ...
    count: int | None = Field(
        None,
        description=(
            "Total number of items available across all pages (not provided by plays endpoint)"
        ),
    ...
    results: list[T] = Field(
        default_factory=list, description="Array of entity objects for the current page"
    )
...
class Host(BaseModel):
    ...
    id: int = Field(..., description="Unique identifier for the host", examples=[1, 42, 123])
    uri: str = Field(
        ...,
    ...
    image_uri: str = Field(
        default="",
        description=(
            "URL to full-size host photo (typically 800x800px), empty string if unavailable"
        ),
    ...
    is_active: bool = Field(
        ..., description="Whether the host is currently active on KEXP", examples=[True, False]
    )
    location: int = Field(
        ...,
...
class Program(BaseModel):
    ...
    id: int = Field(..., description="Unique identifier for the program", examples=[1, 15, 87])
    uri: str = Field(
        ...,
    ...
    name: str = Field(
        ..., description="Program title/name", examples=["Morning Show", "Audioasis", "Overnight"]
    )
    ...
    is_active: bool = Field(
        ..., description="Whether the program is currently broadcasting", examples=[True, False]
    )
    location: int = Field(
        ..., description="Location identifier for the program's broadcast origin", examples=[1, 2]
    )
...
class Show(BaseModel):
    ...
    id: int = Field(
        ..., description="Unique identifier for this show instance", examples=[12345, 67890]
    )
    uri: str = Field(
        ...,
    ...
    program: int = Field(..., description="ID of the parent program", examples=[1, 15, 87])
    ...
    program_name: str = Field(
        ..., description="Name of the parent program", examples=["Morning Show", "Audioasis"]
    )
    ...
    location: int = Field(..., description="Location identifier for the show", examples=[1, 2])
    location_name: str = Field(
        default="", description="Human-readable location name", examples=["Seattle", "Remote"]
    )
...
RotationStatus = Literal["Heavy", "Medium", "Light", "Library", "R/N", "R"]
...
class BasePlay(BaseModel):
    ...
    id: int = Field(..., description="Unique identifier for this play", examples=[1234567, 9876543])
    uri: str = Field(
        ...,
    ...
    show: int = Field(..., description="ID of the show during which this aired", examples=[12345])
    show_uri: str = Field(
        ...,
        description="API endpoint URL for the associated show",
        examples=["https://api.kexp.org/v2/shows/12345/"],
    )
    location: int = Field(..., description="Location identifier", examples=[1, 2])
    location_name: str = Field(
        default="", description="Human-readable location name", examples=["Seattle", "Remote"]
    )
...
class Airbreak(BasePlay):
    ...
    play_type: Literal["airbreak"] = Field(
        default="airbreak", description="Always 'airbreak' for this entity type"
    )
    image_uri: str = Field(default="", description="Always empty for airbreaks")
    thumbnail_uri: str = Field(default="", description="Always empty for airbreaks")
    comment: str = Field(default="", description="Usually empty for airbreaks")
...
class NonTrackPlay(BasePlay):
    ...
    play_type: Literal["nontrackplay"] = Field(
        default="nontrackplay", description="Always 'nontrackplay' for this entity type"
    )
    image_uri: str = Field(default="", description="Always empty for non-track plays")
    thumbnail_uri: str = Field(default="", description="Always empty for non-track plays")
    comment: str = Field(default="", description="Usually empty for non-track plays")
...
class TrackPlay(BasePlay):
    ...
    play_type: Literal["trackplay"] = Field(
        default="trackplay", description="Always 'trackplay' for this entity type"
    )
    ...
    # Artist information
    artist: str = Field(
        ..., description="Artist name", examples=["T. Rex", "The Verve", "The Postal Service"]
    )
    ...
    rotation_status: RotationStatus | None = Field(
        None,
        description=(
            "KEXP rotation status: Heavy (high rotation), Medium, Light, or Library (catalog)"
        ),
...
class Timeslot(BaseModel):
    ...
    id: int = Field(
        ..., description="Unique identifier for this timeslot", examples=[123, 456, 789]
    )
    uri: str = Field(
        ...,
    ...
    # Program association
    program: int = Field(
        ..., description="ID of the program scheduled in this timeslot", examples=[1, 15, 87]
    )
    program_uri: str = Field(
        ...,
    ...
    program_name: str = Field(
        ..., description="Name of the associated program", examples=["Morning Show", "Audioasis"]
    )
    program_tags: str = Field(
        default="",
    ...
    start_date: date = Field(
        ..., description="Date when this timeslot schedule begins", examples=["2025-01-01"]
    )
