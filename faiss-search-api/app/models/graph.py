...
NodeType = Literal["artist", "band", "label", "area", "place", "recording", "work"]
...
class GraphConnectionsRequest(BaseModel):
    ...
    query_type: GraphQueryType = Field(..., description="Type of graph query to execute")
    mbids: list[str] = Field(
        ..., min_length=1, max_length=50, description="MusicBrainz IDs to query (max 50)"
    )
    limit: int = Field(default=20, ge=1, le=100, description="Maximum results per source MBID")
    include_attributes: bool = Field(default=True, description="Include instrument/role attributes")
    # Filter parameters for new queries
    instrument: InstrumentFilter | None = Field(
        default=None, description="Filter by instrument (for members_by_instrument)"
    )
    creator_type: CreatorType | None = Field(
    ...
    collaboration_type: CollaborationType | None = Field(
        default=None,
        description=(
            "Filter by collaboration type (for collaborators_direct): featured, production, writing"
        ),
    )
