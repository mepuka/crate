...
class StoreGeneratedAssetRequest(BaseModel):
    ...
    play_id: Optional[int] = Field(None, description="Play ID (null for artist-level assets)")
    ...
    generation_params: Optional[str] = Field(None, description="Full JSON params for reproducibility")
    image_base64: str = Field(..., description="Base64-encoded image data")
    mime_type: str = Field("image/png", description="MIME type of the image")
    era: Optional[str] = Field(None, description="Detected era (golden-age, classic-rock, etc.)")
    style: Optional[str] = Field(None, description="Style used (art-forward, editorial, etc.)")
    model_notes: Optional[str] = Field(None, description="Notes from the generation model")
    prompt_used: Optional[str] = Field(None, description="Full prompt for debugging")
    gcs_url: Optional[str] = Field(None, description="GCS public URL if uploaded")
