...
class SaveAgentRunRequest(BaseModel):
    ...
    completedAt: Optional[int] = Field(None, alias="completed_at", description="Completion timestamp (Unix ms)")
    status: RunStatus = Field(..., description="Current run status")
    playIds: List[int] = Field(default_factory=list, alias="play_ids", description="Play IDs processed")
    insights: List[InsightSummary] = Field(default_factory=list, description="Insights produced")
    toolCalls: List[ToolCallLogEntry] = Field(default_factory=list, alias="tool_calls", description="Tool calls made")
    researchSteps: List[ResearchStep] = Field(default_factory=list, alias="research_steps", description="Research steps")
    ...
    errorStack: Optional[str] = Field(None, alias="error_stack", description="Error stack trace if failed")
...
class ListAgentRunsParams(BaseModel):
    ...
    playId: Optional[int] = Field(None, alias="play_id")
...
class AgentRunSummary(BaseModel):
    ...
    completedAt: Optional[int] = Field(None, alias="completed_at")
    status: RunStatus
...
class AgentRunDetail(AgentRunSummary):
    ...
    errorStack: Optional[str] = Field(None, alias="error_stack")
