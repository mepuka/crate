...
def publish_enrichment_trigger(
    play_ids: List[int],
    ...
    if batch_id is None:
        batch_id = f"sync_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
...
def publish_enrichment_triggers_batched(
    ...
    if batch_id_prefix is None:
        batch_id_prefix = f"sync_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    ...
    logger.info(
        f"Published {successful} batches for {len(play_ids)} plays "
