    ...
    effect: Effect.gen(function* () {
      ...
      const store = (params: StoreParams): Effect.Effect<ArtifactRef, ArtifactStoreError> =>
        Effect.gen(function* () {
          ...
          Effect.catchAll(e =>
            Effect.fail(new ArtifactStoreError({
              message: `Failed to store artifact: ${e instanceof Error ? e.message : String(e)}`,
              operation: "store",
              cause: e
            }))
          )
...
export const ArtifactStoreServiceScoped: Layer.Layer<ArtifactStoreService> =
  Layer.scoped(
    ...
    Effect.gen(function* () {
      ...
      // Implementation copied from Default for scoped version
      const store = (params: StoreParams): Effect.Effect<ArtifactRef, ArtifactStoreError> =>
        Effect.gen(function* () {
          const id = generateId(params.content)
          const now = new Date().toISOString()
      ...
      const search = (
        ...
        Effect.gen(function* () {
          ...
          const artifacts = params.artifactId
            ? Option.match(HashMap.get(currentStore, params.artifactId), {
                onNone: () => [],
                onSome: (a) => [a]
              })
            : Array.from(HashMap.values(currentStore))
      ...
      const list = (
        ...
        Effect.gen(function* () {
          const currentStore = yield* Ref.get(storeRef)
          let artifacts = Array.from(HashMap.values(currentStore))
      ...
      const clear = (sessionId?: string): Effect.Effect<number> =>
        Effect.gen(function* () {
          const currentStore = yield* Ref.get(storeRef)
          const before = HashMap.size(currentStore)
