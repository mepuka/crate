...
class FaissConfig extends Effect.Service<FaissConfig>()("FaissConfig", {
  effect: Effect.gen(function* () {
    const { baseUrl, timeoutMs } = yield* Config.all({
      ...
      timeoutMs: Config.number("FAISS_TIMEOUT_MS").pipe(
...
const computeStats = (state: GraphState): GraphStats => {
  ...
  for (const edge of state.edges) {
    edgesByType[edge.relationshipType] =
      (edgesByType[edge.relationshipType] || 0) + 1;
  }
...
const program = Effect.gen(function* () {
  ...
  yield* Console.log(
    `  Query time: ${membersResponse.query_time_ms.toFixed(1)}ms`
  );
  ...
  for (const memberMbid of memberMbids) {
    ...
    if (projectsResponse.total > 0) {
      yield* Console.log(
        `    Found ${projectsResponse.total} other projects (${projectsResponse.query_time_ms.toFixed(1)}ms)`
      );
  ...
  if (memberMbids.length > 0) {
    ...
    yield* Console.log(
      `  Querying collaborators for: ${testNode?.name || testMbid}`
    );
    ...
    if (collabResponse.total > 0) {
      yield* Console.log(
        `    Found ${collabResponse.total} collaborators (${collabResponse.query_time_ms.toFixed(1)}ms)`
      );
  ...
  if (originResponse.total > 0) {
    yield* Console.log(
      `  Found origin info (${originResponse.query_time_ms.toFixed(1)}ms):`
    );
  ...
  for (const [type, count] of Object.entries(stats.edgesByType).sort(
    (a, b) => b[1] - a[1]
  )) {
    yield* Console.log(`  ${type}: ${count}`);
  }
 ...
 */
const AppLive = Layer.mergeAll(FetchHttpClient.layer, FaissConfig.Default);
