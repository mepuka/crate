 ...
 *
 * Whisper-quiet utility icons for streaming platforms.
 * Designed to be helpful shortcuts, not advertisements.
 *
 * Philosophy: These are secondary to the music itself.
 * Small, subtle, there when you need them.
 */
...
/**
 * Minimal streaming icon - just the essentials
 */
const StreamingIcon = ({
  link,
  onClick,
  ...
  return (
    <a
      ...
      className={cn(
        ...
        "hover:text-[var(--platform-color)] hover:bg-[var(--platform-color)]/10",
...
/**
 * Loading skeleton - subtle dots
 */
const StreamingIconSkeleton = () => (
  <div className="w-7 h-7 rounded-full bg-foreground/5 animate-pulse" />
);

/**
 * StreamingLinks - Compact inline icon row
 */
export const StreamingLinks = ({
  ...
  // Dedupe by platform, prioritize track > album > artist
  const uniqueLinks = useMemo(() => {
    const kindPriority = { track: 0, album: 1, artist: 2, playlist: 3 };
    ...
    // Group by platform, keep highest priority kind
    const byPlatform = new Map<string, StreamingLink>();
    for (const link of links) {
      const existing = byPlatform.get(link.platform);
      if (!existing || kindPriority[link.kind] < kindPriority[existing.kind]) {
    ...
    return Array.from(byPlatform.values()).sort(
      (a, b) => (platformPriority[a.platform] ?? 99) - (platformPriority[b.platform] ?? 99)
    );
  }, [links]);
